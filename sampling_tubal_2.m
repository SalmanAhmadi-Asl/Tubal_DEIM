...
for i=1:m
    lr(i)=norm(squeeze(U(i,:,:)),'fro')^2;
end
for i=1:n
    lc(i)=norm(squeeze(V(i,:,:)),'fro')^2;
end
for r=1:rr
...
rr=1:r;
semilogy(rr,E_1,'LineWidth',2)
hold on
semilogy(rr,E_2,'LineWidth',2)
hold on
semilogy(rr,E_3,'LineWidth',2)
hold on
semilogy(rr,E_4,'LineWidth',2)
legend('Top tubal leverage scores','Tubal leverage scores sampling','Uniform sampling without replacement','Tubal DEIM');
xlabel('Tubal rank')
ylabel('Error')
